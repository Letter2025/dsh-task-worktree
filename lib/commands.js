...
const USAGE = 'Usage: /worktree mode-on [<name>] | mode-off | create <name> [<base>] [--carry] | list | status [<name>] | finish <name> <message> | bring-back <name> [<message>] | remove <name> [--force] | prune'
 ...
 * @param modeActions - `{ arm(sessionId, name?), disarm(sessionId) }` for
 *   the worktree-mode toggle (may be omitted when the host half is absent).
 */
      ...
      try {
        switch (parsed.kind) {
          ...
          case 'modeOn': {
            ...
            modeActions.arm?.(sessionId, parsed.name)
            const name = parsed.name?.trim()
            return {
              kind: 'success',
              text: name ? `Worktree 模式已启用（分支：${name}）` : 'Worktree 模式已启用（分支名将由 AI 拟定）',
            }
          }
          case 'modeOff': {
            if (sessionId === null) return { kind: 'error', text: 'mode requires an active session' }
            modeActions.disarm?.(sessionId)
            return { kind: 'success', text: 'Worktree 模式已关闭' }
          }
          case 'list': {
