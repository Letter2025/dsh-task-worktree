...
/** Namespace directory inside each repo: worktrees live under <root>/.dsh-worktrees/worktree/. */
const WORKTREE_NAMESPACE = 'worktree'
/** Name = git ref path (branch name). Slashes allowed for hierarchy; no leading/trailing slash, no '..', no '\'. */
const MAX_NAME_LENGTH = 100
// git ref rules, subset: segments of [A-Za-z0-9._-], '/' separators, no empty/./.. segments,
// no segment starting with '.' or '-' or ending with '.', no '@{', no whitespace/control.
function isValidRefName(name) {
  if (typeof name !== 'string' || name.length === 0 || name.length > MAX_NAME_LENGTH) return false
  if (name.startsWith('/') || name.endsWith('/')) return false
  if (name.includes('\\') || name.includes('..')) return false
  if (/[\u0000-\u0020~^:?*[\]]/u.test(name)) return false
  // single-char segments are valid git refs (e.g. branch "x")
  return name.split('/').every((segment) => /^[A-Za-z0-9][A-Za-z0-9._-]*$/u.test(segment))
}

/** Worktree checkout directory for a name: <root>/.dsh-worktrees/worktree/<name segments>. */
function worktreeDirFor(root, name) {
  return path.join(root, '.dsh-worktrees', WORKTREE_NAMESPACE, ...name.split('/'))
}
...
export function createWorktreeManager({ git, dirName = '.dsh-worktrees' }) {
  ...
  async function carryUncommitted(root, worktreePath, signal) {
    // raw=true keeps the diff's trailing newline — trimming it would glue the
    // final hunk to EOF and make `git apply` reject the patch as corrupt.
    const diff = await git(['diff', '--binary', 'HEAD'], root, signal, { raw: true })
    if (diff.exitCode === 0 && diff.stdout.length > 0) {
      // git apply reads its patch from a file argument, never from stdin:
      // a pipe left open by the parent process would block the child forever.
      const patchFile = path.join(root, dirName, `.carry-${process.pid}.patch`)
      await fs.promises.mkdir(path.dirname(patchFile), { recursive: true })
      await fs.promises.writeFile(patchFile, diff.stdout, 'utf8')
      try {
        const applied = await git(['apply', '--whitespace=nowarn', patchFile], worktreePath, signal)
  ...
  return {
    ...
    /**
     * Create a permanent task worktree. The name is both the branch name and
     * the relative path under `<repo>/.dsh-worktrees/worktree/` — e.g.
     * `refactor/logging` yields branch `refactor/logging` at
     * `.../.dsh-worktrees/worktree/refactor/logging`.
     * @param {object} params
     * @param {string} params.name - branch name / relative worktree path (slashes allowed).
    ...
    async create({ name, baseCommit, includeUncommitted = false, cwd, createdBy = null, signal }) {
      ...
      if (!isValidRefName(trimmed)) {
        throw new WorktreeError('INVALID_NAME', `invalid worktree name ${JSON.stringify(name)}: use git ref rules — segments of letters/digits/._- separated by '/', no leading/trailing '/', no '..' or '\\'`)
      }
      const root = await resolveRepoRoot(cwd, signal)
      const target = worktreeDirFor(root, trimmed)
     ...
     * manually with git). Returns the pruned names and the dropped entries
     * (with their paths) so callers can clean up related registrations.
     */
    async prune({ cwd, signal }) {
      const root = await resolveRepoRoot(cwd, signal)
      const manifest = await loadManifest(root)
      if (!manifest) return { repoRoot: root, pruned: [], removedEntries: [] }
      const live = await liveWorktrees(root, signal)
      ...
      const pruned = removedEntries.map((entry) => entry.name)
      if (removedEntries.length > 0) {
        manifest.worktrees = kept
        await saveManifest(root, manifest)
      }
      return { repoRoot: root, pruned, removedEntries }
    },
  }
}